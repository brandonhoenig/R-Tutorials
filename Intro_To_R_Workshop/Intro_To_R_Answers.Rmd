...
flowerData_virginica <- filter(.data = flowerData, 
                               Species == 'virginica')


mean(flowerData_virginica$Sepal.Length)
...
#### Practice Problem 3: Perform a t Test (function: 't.test()') with a confidence interval set to 90% to determine if the pedal width (Pedal.Width) of virginica is greater than that of setosa. Use the '?' to find out the argument name for setting confidence intervals within the t.test() function

```{r eval = FALSE}

t.test(x = flowerData_virginica$Petal.Width, 
       y = flowerData_setosa$Petal.Width,
       conf.level = .9)
...
qplot(x = flowerData$Petal.Length, 
      y = flowerData$Sepal.Length, 
      col = flowerData$Species, 
      ylab = 'Sepal Length',
      xlab = 'Petal Length',
      main = 'My New Pretty Plot') +
  scale_colour_manual(breaks = c('setosa', 'virginica', 'versicolor'),
                      values =  c('black','cyan','orange'))
